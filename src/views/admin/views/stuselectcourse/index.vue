<template>
	...
	<div v-if="!isAdd" class="teacher-table">
		...
		<el-table :data="coursesArr" border style="width: 100%" max-height="700" stripe>
			...
			<el-table-column prop="teacherList" label="授课教师" width="200" align="center">
				<template #="{ row }">
					<el-tag v-for="item in row.teacherList">{{ item.teacherName }}</el-tag>
				</template>
			</el-table-column>
			...
			<el-table-column :filters="[{ text: '未发布', value: '203001' }, { text: '已发布', value: '203002' },]" prop="validDays"
				label="发布状态" width="100" show-overflow-tooltip align="center" :filter-method="filterStatus">
				<template #="{ row }">
...
const getCourses = async () => {
	let result = await reqGetCourseInfoList(pageNo.value, pageSize.value);
